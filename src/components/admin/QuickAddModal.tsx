...
import type { 
  SpiritualRoot, 
  SainsEpigenetikaEntry, 
  KitabKearifanEntry, 
  PopCultureFolkloreEntry 
} from '../../types/root';
...
interface QuickAddModalProps {
  ...
  defaultCategory?: 'sains' | 'kitab' | 'popculture';
  onClose: () => void;
  onSave: (
    rootId: string,
...
export const QuickAddModal: React.FC<QuickAddModalProps> = ({
  ...
  const [selectedRootId, setSelectedRootId] = useState<string>(defaultRootId || roots[0]?.id || '');
  const [category, setCategory] = useState<'sains' | 'kitab' | 'popculture'>(defaultCategory);

  // Common fields
  const [title, setTitle] = useState('');
  
  // Sains fields
  const [field, setField] = useState<'Epigenetika' | 'Neurobiologi' | 'Psikologi Somatik' | 'Fisika Kuantum' | 'Sains Kognitif'>('Epigenetika');
  ...
  const [tradition, setTradition] = useState<'Tasawuf / Islam' | 'Nusantara / Kejawen' | 'Veda / Timur' | 'Hermetisisme' | 'Filsafat Universal'>('Tasawuf / Islam');

  // Pop Culture fields
  const [popType, setPopType] = useState<'film' | 'folklore' | 'novel' | 'figur' | 'anime' | 'simbol'>('film');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (category === 'sains') {
      const entry: SainsEpigenetikaEntry = {
        id,
        title,
        field,
        summary,
      ...
      const entry: KitabKearifanEntry = {
        id,
        title,
        source,
      ...
      const entry: PopCultureFolkloreEntry = {
        id,
        title,
        type: popType,
        characterOrSymbol,
