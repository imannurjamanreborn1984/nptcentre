...
import type { PracticeType, JournalEntry } from '../../types/journal';
import { Sparkles, Save, X } from 'lucide-react';

interface JournalFormProps {
  roots: SpiritualRoot[];
  selectedRoot?: SpiritualRoot;
...
export const JournalForm: React.FC<JournalFormProps> = ({
  ...
  const [breakthroughInsights, setBreakthroughInsights] = useState<string>('');
  ...
  const handleAddCustomSensation = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 p-6 sm:p-8 shadow-sm space-y-6">
      ...
      {/* Somatic Sensations (Pilih sensasi tubuh) */}
      <div>
