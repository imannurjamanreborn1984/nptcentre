...
export const MeditationTimer: React.FC<MeditationTimerProps> = ({
  ...
  const [selectedMinutes, setSelectedMinutes] = useState<number>(defaultDurationMinutes);
  const [timeLeft, setTimeLeft] = useState<number>(defaultDurationMinutes * 60);
  const [isRunning, setIsRunning] = useState<boolean>(false);
  const [breathPhase, setBreathPhase] = useState<'Tarik Nafas' | 'Tahan' | 'Hembuskan' | 'Hening'>('Tarik Nafas');
  ...
  const playChime = () => {
    try {
      ...
      // Tibetan singing bowl frequency simulation (528 Hz Love/DNA repair harmonic)
  ...
  // Rhythmic Breath Guide cycle (4s Tarik - 4s Tahan - 4s Hembus - 4s Hening)
  useEffect(() => {
    if (!isRunning) return;

    const phases: Array<{ phase: 'Tarik Nafas' | 'Tahan' | 'Hembuskan' | 'Hening'; duration: number }> = [
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 p-6 sm:p-8 shadow-sm flex flex-col items-center justify-center text-center relative overflow-hidden">
      ...
      {/* Breathing Guide Pulsing Circle */}
